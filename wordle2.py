...
def blends_digraphs(words):
    subset = [word for word in words if starts_with_blend_or_digraph(word)]
    if len(subset) == 0:
        return words
    return subset

def unqiue_letters(words):
    subset = []
    for word in words:
        seen = set()
        repeat = False
        ...
        if not repeat:
            subset.append(word)
    if len(subset) == 0:
        subset = [word for word in words if all(word.count(char) <= 2 for char in set(word))]
    if len(subset) == 0:
        return words
    return subset
...
def common_letters(words):
    ...
    weird = ['j', 'q', 'x', 'z']
    for word in words:
        if not any(char in word for char in weird):
            subset.append(word)
    if len(subset) ==0:
        return words
    return subset
...
def player1(start="hello", sol="chair"):
    ...
    while output != 'ggggg':
        ...
        if output[2] == 'g' and start[2] in 'aeiou':
            start = word_random(blends_digraphs(words))
        else:
            start = word_random(word_least_vowels(words))
    i = max(1, i-1)
    if print_flag:
        print("Solution found in", i, "guesses.")
...
def player3(sol="chair"):
    ...
    while output != 'ggggg':
        ...
        if print_flag:
            print("#", i, "\tguess:", start, "\toutput", output, "\t words left", len(words))
        i += 1
        start = word_random(word_most_vowels(words))
    i = max(1, i-1)
    if print_flag:
        print("Solution found in", i, "guesses.")
    return i
...
def optimal(sol):
    ...
    while output != 'ggggg':
        ...
        if print_flag:
            print("#", i, "\tguess:", start, "\toutput", output, "\t words left", len(words))
        i += 1
        start = word_random(word_most_vowels(common_letters(unqiue_letters(words))))

    if print_flag:
        print("Solution found in", i-1, "guesses.")
    return i
...
def main(SOL):
    ...
    past = ["BRAVO","BASIN","PAPAL", "CHAOS","GUMMY","MINUS"]
    ...
    opt = [opt.count(i)/len(opt) for i in range(1, 11)]
...
def demo(start, sol):
    ...
    # Optimal (1st: soare, avoid letter repeats, most vowels)
    print("\nOptimal (1st: soare, lots of vowels)")
    optimal(sol)
